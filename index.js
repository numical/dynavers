...
module.exports = function dynavers (specPath) {
  ...
  function specifyModuleVersion (moduleName, version) {
  ...
  function startInterceptingLoad () {
    ...
    Module._load = function (request, parent, isMain) {
      if (specifiedModules[request]) {
        ...
        var index = request.indexOf(path.sep)
        var first = request.slice(0, index)
        if (index > -1 && specifiedModules[first]) {
